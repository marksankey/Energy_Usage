...
def get_electricity_usage_by_time(mpan, serial, use_mock=False):
    ...
    if use_mock:
        return {
            'off_peak_usage': 6.2,  # 23:30-05:30
            'peak_usage': 2.3,      # 05:30-23:30
            'total_usage': 8.5
        }
    ...
    try:
        ...
        if not results:
            return {'off_peak_usage': 0, 'peak_usage': 0, 'total_usage': 0}
        ...
        for reading in results:
            ...
            # Convert to local time (assuming UK timezone)
            hour = interval_start.hour
            minute = interval_start.minute
            
            # Off-peak: 23:30-05:30 (includes 23:30-00:00 and 00:00-05:30)
            if (hour == 23 and minute >= 30) or (hour < 5) or (hour == 5 and minute < 30):
        ...
        return {
            ...
            'total_usage': round(off_peak_usage + peak_usage, 2)
        }
        
    except Exception as e:
        print(f"Error fetching electricity data: {e}")
        return None
...
def index():
    ...
    <body style="font-family: Arial; margin: 40px;">
        <h1>TRMNL Energy Plugin Test</h1>
        <h2>Test Links:</h2>
        <ul>
            ...
            fetch('API_URL_PLACEHOLDER')
                ...
                .then(data => {
                    ...
                    document.getElementById('content').innerHTML = 
                        ...
                        '<div class="grand-total">Daily Total: £' + data.total_cost.toFixed(2) + '</div>' +
                        '<div class="footer">' + (data.mock_data ? 'Mock Data' : 'Live Data') + '</div>';
                })
                .catch(error => {
